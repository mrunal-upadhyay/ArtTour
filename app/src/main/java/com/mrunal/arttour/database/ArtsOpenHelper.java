package com.mrunal.arttour.database;
...
public class ArtsOpenHelper extends SQLiteOpenHelper {
  ...
  public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
    Log.w("ArtsOpenHelper", "Upgrading database from version " + oldVersion + " to " + newVersion
        + ". OLD DATA WILL BE DESTROYED");

    // Drop the table
    db.execSQL("DROP TABLE IF EXISTS " + ArtsContract.ArtsEntry.TABLE_ARTS);
    db.execSQL(
        "DELETE FROM SQLITE_SEQUENCE WHERE NAME = '" + ArtsContract.ArtsEntry.TABLE_ARTS + "'");
